...
The other I/O function defined in `simUtil.ml` is `file_as_string : string -> string`: given a file name, it returns the entire
...
Our distance mechanism treats text documents as _bags_ of _n-grams_. We'll describe bags, also called multisets, below. _n-grams_ are sequences of n consecutive letters from a string, so for example, in the string "Bazinga", the 3-grams are "Baz", "azi", "zin", "ing", and "nga". (We'll use n=4, which we've declared at the top of `similar.ml`, for this assignment) We'll build a function to extract "normalized" n-grams from a string in three steps:
...
evaluate to `["cont"; "onti"; "ntin"; "tinu"; "inue"; "nued"; "almo"; "lmon"; "mond"; "milk"; "coff"; "offe"; "ffee"]` and `n_grams "DRESS BENCH!"` should evaluate to `["dres"; "ress"; "benc"; "ench"]`. Remember, use `let`
